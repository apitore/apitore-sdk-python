# coding: utf-8

"""
    Twitter APIs with Word2Vec query expansion, and add sentiment by Apitore sentiment API

    Call Twitter APIs with word2vec query expansion and sentiment analysis.<BR />[Endpoint] https://api.apitore.com/api/26  # noqa: E501

    OpenAPI spec version: 0.0.1
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from setuptools import setup, find_packages  # noqa: H301

NAME = "swagger-client"
VERSION = "1.0.0"
# To install the library, run the following
#
# python setup.py install
#
# prerequisite: setuptools
# http://pypi.python.org/pypi/setuptools

REQUIRES = ["urllib3 >= 1.15", "six >= 1.10", "certifi", "python-dateutil"]

setup(
    name=NAME,
    version=VERSION,
    description="Twitter APIs with Word2Vec query expansion, and add sentiment by Apitore sentiment API",
    author_email="",
    url="",
    keywords=["Swagger", "Twitter APIs with Word2Vec query expansion, and add sentiment by Apitore sentiment API"],
    install_requires=REQUIRES,
    packages=find_packages(),
    include_package_data=True,
    long_description="""\
    Call Twitter APIs with word2vec query expansion and sentiment analysis.&lt;BR /&gt;[Endpoint] https://api.apitore.com/api/26  # noqa: E501
    """
)
